...
def get_sharegpt(split: str, silent: bool = False, cache_dir: str = None, num_turns: int = 2, data_fraction: float = 1.0) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    return get_sharegpt_internal(split, silent, cache_dir, num_turns, data_fraction, 
                                 'ShareGPT_V3_unfiltered_cleaned_split_no_imsorry.json')

def get_zelbrevity(split: str, silent: bool = False, cache_dir: str = None, num_turns: int = 2, data_fraction: float = 1.0) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    return get_sharegpt_internal(split, silent, cache_dir, num_turns, data_fraction, 
                                 f'adapted-{split}-gpt4o-arena-brevity-dpo.json')

def get_sharegpt_internal(split: str, silent: bool = False, cache_dir: str = None, num_turns: int = 2, 
                          data_fraction: float = 1.0, filename: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the ShareGPT dataset (needs to be local json file).
       ...
       Multiple turns are allowed, but the prompt should always start with \n\nHuman: and end with \n\nAssistant:.
    """
    full_path = os.path.join(cache_dir, 'sharegpt_data', filename)
    print(f'Loading the ShareGPT dataset: {full_path}')
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None, **kwargs):
    ...
    elif name == 'zelbrevity':
        data = get_zelbrevity(split, silent=silent, cache_dir=cache_dir)
    else:
